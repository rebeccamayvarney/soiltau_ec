...
for model_i in range(0, n_models):
        ...
        print(poly_relationship_model)

        ax.plot(sorted_x, poly_relationship_model(sorted_x), model_colours[model_i], linewidth=5)#, label=model)
        #ax.legend(loc=9, fontsize=30) # change location for best fit
...
#['darkblue', 'dodgerblue', '#80b1d3', 'lightblue', '#8dd3c7', 'darkseagreen', 'darkgreen', 'olive', 'gold', 'orange', 'peachpuff', '#fb8072', 'red', 'hotpink', '#fccde5', '#bebada'] 
        ...
        for model_j in range(0, n_models):
                ...
                if model_j > 6:
                        ...
                        if current_ssp == 'ssp126':
                                ax.plot(x_value_cmip5, y_value_cmip5, marker='o', color=model_colours[cmip5_marker], markersize=20, mew=5)#marker=model_shapes[cmip5_marker], color='r', markersize=20, mew=5)
                        elif current_ssp == 'ssp245':
                                ax.plot(x_value_cmip5, y_value_cmip5, marker='^', color=model_colours[cmip5_marker], markersize=20, mew=5)#marker=model_shapes[cmip5_marker], color='g', markersize=20, mew=5)
                        elif current_ssp == 'ssp585':
                                ax.plot(x_value_cmip5, y_value_cmip5, marker='s', color=model_colours[cmip5_marker], markersize=20, mew=5)#marker=model_shapes[cmip5_marker], color='b', markersize=20, mew=5)
                        ...
                        # plotting
                        if current_ssp == 'ssp126':
                                ax.plot(x_value_cmip6, y_value_cmip6, marker='o', color=model_colours[model_j], markersize=20, mew=5)#marker=model_shapes[model_j], color='r', markersize=20, mew=5)
                        elif current_ssp == 'ssp245':
                                ax.plot(x_value_cmip6, y_value_cmip6, marker='^', color=model_colours[model_j], markersize=20, mew=5)#marker=model_shapes[model_j], color='g', markersize=20, mew=5)
                        elif current_ssp == 'ssp585':
                                ax.plot(x_value_cmip6, y_value_cmip6, marker='s', color=model_colours[model_j], markersize=20, mew=5)#marker=model_shapes[model_j], color='b', markersize=20, mew=5)


one_to_one_line = np.linspace(-850,50,100) # one to one line
