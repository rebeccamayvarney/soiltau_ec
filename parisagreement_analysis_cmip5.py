...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb  6 14:21:20 2020
...
#regrid_cube = open_netCDF(regrid_cube)
regrid_cube = select_time(regrid_cube, 2001, 2010)
regrid_cube = time_average(regrid_cube)
#regrid_cube = iris.util.squeeze(regrid_cube)
...
for temp_option in range(0, temperature_change_options_length):
    ...
    for rcp_option in range(0, rcp_options_length):
        ...
        for model_i in range(0, n_models):
            ...
            tas_test_years = tas_test_cube.coord('year').points # defining the time variable for years
            ...
            tas_new_cube = time_average(tas_new_cube) # time average
            save_model_temp_cube = tas_new_cube.copy() # saving future temperature spatial cube
            save_model_temp_data = tas_new_cube.data - 273.15 # saving future temperature spatial data
            ...
            y_array[temp_option, rcp_option, model_i] = actual_delta_Cs_data
            ...
            # Calculating new tau_s with relationship f
            ...
            #historical_rh_cube = numpy_to_cube(historical_rh, cSoil_cube, 2)
            #historical_rh_regrid = regrid_model(historical_rh_cube, regrid_cube)
            #historical_rh_regrid = historical_rh_regrid.data
            
            delta_tau_obs = ma.exp(future_tau_obs) - ma.exp(historical_tau_obs)
            delta_cSoil_obs = delta_tau_obs*observational_rh #observational_rh#historical_rh_regrid
